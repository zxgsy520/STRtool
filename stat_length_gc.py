...
def stat_base_length(genome, file):

     if file=="":
         depth_dict = {}
         LOG.info("No depth file")
     else:
     ...
     for seq_id, seq in fh:
         ...
         if seq_id not in depth_dict:
             ...
             cover = depth_dict[seq_id]/length

         print('{}\t{}\t{:.2f}\t{}'.format(seq_id, length, (g+c)*100.0/length, cover))
     return 0
...
def main():
    ...
    parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,
        description="""
name:
    stat_length_gc.py  Statistical sequence length, GC content and depth of coverage

attention:
    stat_length_gc.py  genome.fa > length_gc.tsv
    ----Only count the length of the statistical sequence and the GC content
    stat_length_gc.py genome.fa -d genome.depth > length_gc.tsv
    ----Statistical sequence length, GC content and depth of coverage
...
contact:  %s <%s>\
    ...
    stat_base_length(args.input, args.depth)
